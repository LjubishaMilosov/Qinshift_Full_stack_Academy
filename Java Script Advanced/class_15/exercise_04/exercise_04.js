...
function Movie(title, genre){
    ...
    this.addCinema = function(cinema){
        ...
        // we canot add the same cinema again
        ...
        //if there is not, then add this movie tothat list
        if(cinema.movies.filter(m => m.title === this.title).length = 0){
            cinema.movies.push(this);
        }
    }
    this.removeCinema = function(cinema){
        // if there is not an item i nthe result array, then a cinema with that name does not exist
        ...
        // if movie with this title exists in the cinema, remove it
        if(cinema.movies.filter(m => m.title === this.title).length > 0){
            for(let i =0; i < cinemma.movie.length; i++){
